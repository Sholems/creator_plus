    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <span className="rounded-full bg-forest-100 px-3 py-1 eyebrow text-[0.625rem] text-forest-700">
            {product.category?.name}
          </span>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2">
            ...
            {product.averageRating ? (
              ...
              <span className="text-sm text-ink-400">New on CreatorPlus</span>
            )}
            {salesCount > 0 && (
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 rounded-2xl border border-ink-100 bg-white p-6 shadow-[0_1px_2px_rgba(22,33,27,0.04)]">
            <div className="flex items-end justify-between">
              <div>
                <p className="eyebrow text-ink-400">{product.licenseType || 'Personal'} license</p>
                <p className="price-tag mt-1 text-3xl font-bold text-forest-900">{formatNaira(product.price)}</p>
              </div>
            ...
            <button
              ...
              className="mt-6 w-full rounded-full bg-forest-800 px-4 py-3.5 text-sm font-semibold text-cream-50 shadow-sm transition-colors hover:bg-forest-700 disabled:opacity-50"
            >
            ...
            <button
              ...
              className="mt-3 w-full rounded-full border border-forest-300 bg-white px-4 py-3.5 text-sm font-semibold text-forest-800 transition-colors hover:bg-cream-100 disabled:opacity-50"
            >
              {isAddingToCart ? 'Adding…' : 'Add to Cart'}
            </button>

            {cartMessage && (
              <p className="mt-2 text-center text-sm font-medium text-forest-700">{cartMessage}</p>
            )}
              ...
              <div className="mt-6 rounded-2xl border border-ink-100 bg-cream-50/60 p-4">
                <div className="flex items-center gap-3">
                  {product.creator.slug ? (
                    ...
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-cream-100 ring-1 ring-ink-100">
                      <span className="font-display text-base font-bold text-forest-700">
                        {product.creator.storeName[0]}
                      </span>
                    </div>
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="eyebrow text-[0.625rem] text-ink-400">Sold by</p>
                    {product.creator.slug ? (
                      <Link
                        href={`/creator/${product.creator.slug}`}
                        className="block truncate font-display text-sm font-semibold text-forest-800 transition-colors hover:text-forest-600 hover:underline"
                      >
                        {product.creator.storeName}
                      </Link>
                    ) : (
                ...
                {product.creator.slug && (
                  <Link
                    href={`/creator/${product.creator.slug}`}
                    className="mt-3 block w-full rounded-full border border-forest-300 bg-white px-4 py-2 text-center text-xs font-semibold text-forest-800 transition-colors hover:bg-cream-100"
                  >
                    Visit the shop
                  </Link>
                )}
            ...
            <div className="mt-6 space-y-3 text-sm text-ink-600">
              {[
                ...
                <div key={row.text} className="flex items-center gap-2.5">
                  <svg className="h-5 w-5 shrink-0 text-gold-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="mt-6 border-t border-ink-100 pt-4">
              <h3 className="font-display text-sm font-semibold text-ink-900">Product details</h3>
              <dl className="mt-3 space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <dd className="text-ink-900 capitalize">{product.licenseType}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-ink-400">Version</dt>
                  <dd className="text-ink-900">{product.currentVersion || product.version || '1.0'}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-ink-400">Sales</dt>
                  <dd className="text-ink-900">{salesCount}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-ink-400">Downloads</dt>
                  <dd className="text-ink-900">{product.downloadCount || 0}</dd>
                </div>
                <div className="flex justify-between">
