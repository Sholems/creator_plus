...
export default function NewProductPage() {
  ...
  const [affiliate, setAffiliate] = useState({ affiliateEnabled: false, affiliateCommissionRate: 20 });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (descriptionPlain.length < 50) {
      setError(`Description must be at least 50 characters (currently ${descriptionPlain.length}).`);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (digitalFile) {
        try {
          await api.uploadProductFile(token, product.id, digitalFile);
        } catch (uploadErr: any) {
          setError(
            `Product was created but the digital file failed to upload: ${
      ...
      <form onSubmit={handleSubmit} noValidate className="mt-8 max-w-3xl">
        <div className="space-y-6">
          <div className="rounded-2xl border border-ink-100 bg-white p-6 shadow-[0_1px_2px_rgba(22,33,27,0.04)]">
            ...
            <div className="mt-4 space-y-4">
              ...
              <div>
                ...
                <p className={`mt-1 text-xs ${descriptionPlain.length >= 50 ? 'text-forest-600' : 'text-ink-500'}`}>
          ...
          <div className="rounded-2xl border border-ink-100 bg-white p-6 shadow-[0_1px_2px_rgba(22,33,27,0.04)]">
            ...
            <p className="mt-1 text-sm text-ink-500 mb-4">Upload a preview image for your product (PNG, JPG, max 5MB)</p>

            {thumbnailPreview ? (
              <div className="relative">
                <img src={thumbnailPreview} alt="Thumbnail preview" className="h-48 w-full rounded-xl object-cover" />
                <button
                  type="button"
          ...
          <div className="rounded-2xl border border-ink-100 bg-white p-6 shadow-[0_1px_2px_rgba(22,33,27,0.04)]">
            ...
            <p className="mt-1 text-sm text-ink-500 mb-4">
              Upload the file buyers will receive after purchase (PDF, ZIP, images, templates — max 500MB).
            </p>

            {digitalFile ? (
              <div className="flex items-center justify-between rounded-xl border border-forest-200 bg-forest-50 p-4">
              ...
              <label className="flex flex-col items-center justify-center h-32 cursor-pointer rounded-xl border-2 border-dashed border-ink-200 bg-cream-50 transition-colors hover:border-forest-400">
                <svg className="h-8 w-8 text-ink-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                    d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                </svg>
                <span className="mt-2 text-sm text-ink-600">Click to upload your digital product file</span>
                <input
                  type="file"
        ...
        <div className="mt-8 flex items-center justify-end gap-4">
          ...
          >
            {isLoading ? (uploadingThumbnail ? 'Uploading thumbnail…' : 'Creating…') : 'Create Product'}
          </button>
        </div>
      </form>
    </div>
